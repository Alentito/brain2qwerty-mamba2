...
# Brain2Qwerty V3 — Hybrid Mamba-2 (SpanishBCBL)

V3 replaces V2's Conformer sequence core with a Nemotron-H-style hybrid
Mamba-2 / attention stack (`mamba.py`: pure-PyTorch SSD implementation, no
`mamba-ssm` dependency), and retargets the V2 pipeline (CTC + word-contrastive
+ LoRA-LLM decoding) from EnglishBCBL to **SpanishBCBL** (`Pinet2024Meg`), with
the V1 TF-IDF paraphrase-cluster split (seed 1) so results are directly
comparable to the reproduced V1 baseline (test CER 0.389).

Everything else — data pipeline, staged losses, metrics, decoding — is
identical to V2. See `Project_Notes/V3_Verification_Checklist.md` for the
test/run order.
