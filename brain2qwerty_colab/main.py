...
from . import models as _models  # noqa: F401  (registers ConvMambaEncoder)
...
from .utils import MegChannelPositions2D, accelerator, build_events, prepare_word_embeddings

log = logging.getLogger(__name__)


class Data(pydantic.BaseModel):
    """Sentence-level dataloaders for the combined pipeline.

    Runs the study, applies the combined V1+V3 preprocessing/split/word
    transforms, extends each sentence window by a small random tail, and builds
    one padded dataloader per split (train-time MEG onset jitter is applied
    inside the dataset). Subject subsetting lives in
    ``CombinedBCBLPreprocessing(subjects=...)``.
    """
    ...
    def build(self) -> dict[str, DataLoader]:
        ...
        chan_pos = MegChannelPositions2D(neuro=self.neuro)
...
class Experiment(pydantic.BaseModel):
    """Train and evaluate the combined Brain2Qwerty end-to-end pipeline."""
    ...
    def _trainer_setup(self) -> pl.Trainer:
        ...
        if self.eval_only:
            # Evaluate in a single process: the prediction callback then captures
            # the whole test split with no DDP sharding to reconcile.
            devices = 1
...
def _parse_subjects(raw: list[str] | None) -> list | None:
    """Parse ``--subjects S1 S3`` / ``--subjects 1 3`` into a subject list."""
    if not raw:
        return None
    out: list = []
    for s in raw:
        out.append(int(s) if s.lstrip("Ss").isdigit() and not s.startswith(("S", "s")) else s)
    return out
...
def main(argv: list[str] | None = None) -> None:
    ...
    ``python -m brain2qwerty_colab.main {debug,train,colab,eval,cache}`` — every
    command is the same ``Experiment``, only the config (and eval/cache mode)
    differs. ``--subjects`` selects a subset of the 19 participants.
    """
    import argparse

    from . import studies  # noqa: F401  (registers the Pinet2024Meg study)

    from .cli import add_wandb_args, wandb_config
    from .config.xp_config import colab_config, debug_config, experiment_config

    parser = argparse.ArgumentParser(prog="brain2qwerty_colab")
    ...
    p_colab = sub.add_parser(
        "colab", help="single-GPU training preset for Colab/Kaggle notebooks"
    )
    p_colab.add_argument("--resume", default=None, help="checkpoint to resume from")
    p_colab.add_argument("--seed", type=int, default=None, help="override the seed")
    p_colab.add_argument(
        "--small", action="store_true", help="use the 512-dim small encoder variant"
    )
    ...
    for p in (sub.choices["debug"], p_train, p_colab, p_eval):
        add_wandb_args(p)
    for p in (sub.choices["debug"], p_train, p_colab, p_cache):
        p.add_argument(
            "--subjects",
            nargs="+",
            default=None,
            metavar="S",
            help="subset of participants, e.g. --subjects S1 S3 S7 (default: all 19)",
        )
    args = parser.parse_args(argv)
    subjects = _parse_subjects(getattr(args, "subjects", None))

    if args.command == "cache":
        cfg = debug_config(subjects) if args.debug else experiment_config(subjects)
        print("[brain2qwerty_colab] pre-warming the feature cache...")
        Experiment(**cfg).data.build()
        print("[brain2qwerty_colab] cache warmed.")
        return

    if args.command == "debug":
        cfg = debug_config(subjects)
    elif args.command == "colab":
        cfg = colab_config(subjects, small=args.small)
    else:
        cfg = experiment_config(subjects)
    ...
    print(
        f"[brain2qwerty_colab] running in '{args.command}' mode "
        f"(seed={cfg.get('seed')}, subjects={subjects or 'all 19'})"
    )
    Experiment(**cfg).run()
